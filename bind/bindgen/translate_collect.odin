...
translate_collect :: proc(filename: string, config: Config, types: Type_List, decls: Decl_List) -> (Translate_Collect_Result, bool) {
	...
	// Append UNIX clang include paths.
	append(&clang_args, fmt.ctprintf("-I/usr/lib/clang/%s/include/", clang_major_version_str))
	append(&clang_args, fmt.ctprintf("-I/usr/lib/llvm%s/lib/clang/%s/include/", clang_major_version_str, clang_major_version_str))
